...
OUTDIR = f"figures/supplement/out/nonlinear_manifolds/latent_geometry"
SAVEPLOTS = True
...
CONTOUR_LEVELS = [2,4,8,16,32,64,128,150,160,165,168,169,170,171]
...
ax.set_title("Latent space potential $\phi$")
ax.set_xlabel("$u$")
ax.set_ylabel("$v$")
#ax.set_xticks([])
#ax.set_yticks([])
...
ax.set_title("Transformed potential $\\varphi$")
ax.set_xlabel("$x'$")
ax.set_ylabel("$y'$")
ax.set_xticks([-2, 0, 2])
ax.set_yticks([-2, 0, 2])
...
ax.quiver(
    xs_vect.flatten(), ys_vect.flatten(), 0*zs_vect.flatten() + zs.min() - 5, 
...
ax.set_title(f"Transformed dynamics $F$")
ax.set_xlabel("$x'$")
ax.set_ylabel("$y'$")
ax.set_xticks([-2, 0, 2])
ax.set_yticks([-2, 0, 2])
